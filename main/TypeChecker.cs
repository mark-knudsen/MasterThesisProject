...
namespace MyCompiler
{
    public class TypeChecker : ITypeVisitor
    {
        ...
        public Type VisitWhere(WhereNodeExpr expr)
        {
            ...
            {
                var recordArguments = new List<NamedArgumentNodeExpr>();
                for (int i = 0; i < dfType.ColumnNames.Count; i++)
                {
                    if (dfType.DataTypes[i] is StringType)
                        recordArguments.Add(new NamedArgumentNodeExpr(dfType.ColumnNames[i], new StringNodeExpr("")));
                    else if (dfType.DataTypes[i] is BoolType)
                        recordArguments.Add(new NamedArgumentNodeExpr(dfType.ColumnNames[i], new BooleanNodeExpr(false)));
                    else
                        recordArguments.Add(new NamedArgumentNodeExpr(dfType.ColumnNames[i], new NumberNodeExpr(0)));
                }

                var record = new RecordNodeExpr(recordArguments);

                Visit(new AssignNodeExpr(expr.IteratorId.Name, record));
        ...
        public Type ResolveType(TypeNodeExpr node)
        {
        ...
        {
            if (expr is TypeLiteralNodeExpr t)
            {
        ...
        public Type VisitTypeLiteral(TypeLiteralNodeExpr expr)
        {
